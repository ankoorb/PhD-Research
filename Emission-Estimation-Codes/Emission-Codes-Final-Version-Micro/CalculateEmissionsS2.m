...
nvc = size(N,2);
Vinfo = zeros(TP,nvc+1,4);
parfor tpe=1:TP % Loop on all time periods
    s2c=int2str(tpe);
    % Load file with trajectories for all vehicle classes for time period
    sn=['trajx.bin_out_time_',s2c];
    fid=fopen(sn);
    ...
    % Calculate emissions
    [Veh_stat]=RunOpMode_MOVES_S2(s2c,traj,N,cvc,Op_rate,SegmentLink,modes,param,veh_type); % Calculate emissions          
    Vinfo(tpe,1:nvc,1:4)=Veh_stat(1:nvc,1:4);
end
