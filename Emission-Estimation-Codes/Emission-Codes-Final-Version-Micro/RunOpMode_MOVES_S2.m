...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [VehStat]=RunOpMode_MOVES_S2(s2c,traj,N,cvc,Op_rate,SegmentLink,modes,param,veh_type);

VehStat=zeros(size(N,2),4);
...
for ii=1:size(N,2) % Loop on vehicle classes
    ...
    if numel(Mdat)>0
        ...
        %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        % 0. Load data for vehicle class (vehicle IDs, segment IDs, vehicle speeds
        ...
        VehStat(ii,:)=[Veh_count VMT VHT TCT];
	...
	csvwrite(s4,link_emi2);
        fprintf('Done with vehicle class %d, time period %c \n',VehClass,s2c)

    else % Case when there are no data to process
        VehStat(ii,:)=[0 0 0 0];
        fprintf('No valid trajectory for vehicle class %d in time period %c \n',VehClass,s2c)
