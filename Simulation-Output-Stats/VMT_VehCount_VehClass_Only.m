% Calculate VMT based on Veh Class (only) and Vehicle Count based on Road Class and Vehicle Class 
% Author: Ankoor
% Date: Nov 19, 2013
...
% VMT Calculation Based on Vehicle Class Starts
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

distance(:,1)=[];
distance=sortrows(distance,1); % Sort generated unique VehID in ascending order
[vmt_set,fref]=unique(distance(:,1),'first'); % Find index when unique road type appears first time
[vmt_set,lref]=unique(distance(:,1),'last'); % Find index when unique road type appears last time

    for i=1:size(vmt_set,1)   
        temp_veh=distance(fref(i):lref(i),:);
        InfoA(i,1)=[temp_veh(size(temp_veh,1),2)-temp_veh(1,2)];      
    end
    VMT(a,1)=sum(InfoA);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% VMT Calculation Based on Vehicle Class Ends
