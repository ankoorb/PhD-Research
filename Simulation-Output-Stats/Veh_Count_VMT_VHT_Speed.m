...
% Calculate Vehicle Count, VMT, VHT, Speed = VMT/VHT based on Vehicle Class
clear all; clc;
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
tic
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
simul_time=0 % simulation start time from midnight (sec) Enter 0
clearance_time=25;
...
% Find queued and enroute vehicles
...
check8=check8.*-1; 
temp_check8=temp_check8+check8; % "-ve" values for vehicle with status = missed 
...
new_trip=sortrows(new_trip,2); % Sort generated unique vehicle class in ascending order
[new_path_set,fref]=unique(new_trip(:,2),'first'); % Find index when unique vehicle class appears first time
[new_path_set,lref]=unique(new_trip(:,2),'last'); % Find index when unique vehicle class appears last time
...
% Excel output
header={'VehClass','VehCount','VMT','VHT','Speed',};
xlswrite('VehicleCount.xlsx',header,'Sheet1','a1');
xlswrite('VehicleCount.xlsx',Info,'Sheet1','a2');
