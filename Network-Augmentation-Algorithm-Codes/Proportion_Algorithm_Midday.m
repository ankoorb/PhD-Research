...
for g=1:1  
    ...
    TC_D=load(s4);
    TC_O=load(s3);
    for h=1:24 %number of Gunwoo's 15min matrix, 2 for example.
    s0=int2str(h);
    ...
    for a=1:i   %common node 1-i
        for d=1:Proportion_AM_O_size-1 %row of TC_O
            if TC_O(d+1,1)==Common(a,1)
               for b=1:455:p-1 %first column of Ankoor OD matrix, 455 is the #of Ankoor nodes
                   if Ankoor(b+1,1)==Common(a,1)
                      for c=b:b+454 %second column of Ankoor OD matrix,455 #of nodes in Ankoor OD matrix
    ...
    for a=1:i   %common node 1-i
        for d=1:Proportion_AM_D_size-1 %row of TC_O
            if TC_D(d+1,2)==Common(a,1)
               for b=1:455 %second column of Ankoor OD matrix, 6 for example, 455 is the #of Ankoor nodes
                   if Ankoor(b+1,2)==Common(a,1)
                      for c=b:455:p-1 %first column of Ankoor OD matrix,6 for example 455 for Ankoor OD matrix
...
for g=1:1  
    ...
    for h=1:24 %number of Gunwoo's 15min matrix                             
        s0=int2str(h);
                    ...
                    for c=1:i  %% common node 1-i 
                        for d=b:b+269 %% Gunwoo second row, 6 for example, 269 for #of Gunwoo node
                              if Gunwoo(d+1,2)==Common(c,1)
                                  for f=1:455:p-1  %% Ankoor first row, 6 for example, 455 for ankoor
                                      if Ankoor(f+1,1)==Common(a,1)
                                          for k=f:f+454 %% Ankoor second row, 6 for example, 455 for ankoor
    ...
    for h=1:24 %number of Gunwoo's 15min matrix                             
        ...
        [w,v]=size(Gunwoo);
            ...
            for c=1:6:j-2  %# of query
                for r=1:2946  %number of queries
            ...
            for r=1:i-1 %first column of TC_2link
                for a=1:455:p-1 % 6 for example, 455 for Ankoor
                    if Ankoor(a+1,1)==TC_2link(r+1,1)
                        for b=a:a+454 % 455 for Ankoor
